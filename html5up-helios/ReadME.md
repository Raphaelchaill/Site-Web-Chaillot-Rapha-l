...
j'ai choisi pour la page de garde de mettre un image de temple grec afin d'atirer le regard et donner un effet de profondeur sur le titre, au centre de la page, mais légèrement surélevé afin de favoriser la lisibilité. J'ai tenter plusieurs images et plusieurs couleurs car la lisibilité posait problème à cause de l'image. Cela m'a mené à légèrement flouté l'image et à mettre le titre en gras.
La barre supérieur possède 3 redirection. celle du centre mène sur la page centrale (la page actuelle) la redirection "bibliographie" mène à mon travail de recherche bibliographique sur mon sujet. Enfin l'onglet "Sources" mène à un tableau qui présente les sources que j'ai pu étudier.
...
''' #header {
		...
		cursor: default;
```
Les caractéristiques de la page de garde ont été modifié dans le CSS, j'ai choisi la couleur #FFD700 similair à de l'or vieilli car le mariage or-blanc est élégant.j'ai surtout modifié la couleur, l'image et le fait que les caractères soient en gras.
...
Après cela je me suis attelé à présenté plus en détail Platées et sa place dans Thucydide, auteur principal de mon corpus. J'ai pris garde à laissé des espaces entres les textes et les schémas pour davantage de lisibilité.

Pour illustrer mes propos, j'ai utilisé le site Voyant Tools pour faire des nuages de mots. Je les ais inclus dans mon site web grace au lien que fournis Voyants Tools, cela permet de garder l'interactivité des schémas, contrairement à un simple capture d'écran. 
...
Néanmoins l'inconvénient est que ces diagrammes peuvent mettrent un certain temps pour s'afficher, de plus intéragir avec eux, en mettant par exemple la souris sur un mot pour voir le nombre d'itérations créer un clignotement de la fenêtre qui apparaît.

le "*main*" se termine par une explication final de l'objectif de mon mémoire. dans celui ci j'ai introduit un lien vers la page "sources" de mon site web par commodité et pour éviter à l'utilisateur de remonter en haut de la page.
...
Au vu du peu de temps que j'ai pu utiliser pour apprendre javascript, j'ai dû m'aider de l'inteligence artificielle générative pour m'assister lorsque j'ai rencontré des problèmes. Je préfère vous le signaler, car même si l'IA générative est un outil vital pour tout codeur de demain je comprend que son utilisation peut vous poser problème. Néanmoins j'ai tout fait pour que ses interventions me soit autant utiles qu'instructives.
...
```
Tout d'abord j'ai définit le style, la forme et l'utilisation de la boite modal. Cette partie aurait normalement sa place dans le CSS, mais j'ai préféré la marquer au début de mon code HTML, car le site étant fait sur des qui me sont étrangère, j'ai considéré que le mettre dans mon HTML me permettait d'avoir plus de contrôle et d'éviter des problèmes chronophages. Bien que le template à de grands avantages, creer aussi à aussi les siens, bien que j'aurais aimé le faire de A à Z, le temps me manquait.

afin d'éviter de surcharger mon site web vers d'innombrable redirections vers d'autres pages web, j'ai priviligié des boites modales pour afficher ma chronologie et des cartes diverses. le code ci dessus permet -entre autres- de définir les moyens d'ouverture et de fermeture de la boite modale, la taille de la boite modale sur la page, il inclut un arrière plans sombre et met en place des flèches pour faire défiler les images. Au début j'ai rencontré des problèmes car les images dépassaient de la boite modale et étaient illisible, c'est pourquoi j'ai décidé de réduire la taille maximale  de la boite modale à 80% de la taille de la page.

le script de la première boite modale est plutôt simple et consiste à ouvrir la boite modal et à la fermer Selon son style définit plus haut.

une image différente de celle affiché apparaît lorsque l'utilisateur clique dessus. La boite modale disparâit lorsque l'utilisateur clique en dehors de celle-ci.
```java
'''<script> 
			function openModal() {
...
J'ai donc créer une liste des images avec leur url. l' "index" permet de faire défiler les images grâce à une variable. 
```java
'''	const mapImages = [
...
```
Après avoir ouvert la boite modale, le script met en place un ordre pour chaque image (j'ai choisis de les trier de la plus à la moins pertinente). La fonction "updrateModaleImage" permet de mettre à jours l'affichage.
...
Par ailleurs afin d'améliorer l'élégence de mon site web, j'ai ajouter un "favicon" présent sur toute les pages de mon site web. L'icone se veut similaire à une forme de monnaie, c'est pourquoi j'ai inscrit les initiales de la cité de Platées : ΠΛΑ.
...
Cette page est la plus simple des 3, elle consiste à repertorier toute la bibliographie que j'ai rassemblé jusqu'a présent.

![extrait bibliographie](image-3.png)

Cette partie n'a pas nécessiter de codage particulier, si ce n'est pour mettre les auteurs en gras et les oeuvres en italique.

## La page "Sources"

Cette page est la plus complexe de mon projet, et j'ai eu beaucoup de difficulté à la mettre en place. Malheuresement, ma volonté de faire un tableau de mes sources interactif était en dehors de mes moyens, je me suis la encore appuyé sur l'IA pour m'aider à coder, mais de manière plus importante. Le code dépasse en grande partie ma compréhension.  Mais je voulais obtenir un outil qui me soit utile pour mon mémoire, et c'est chose faite. Je comprendrais néanmoins que cette partie ne soit pas pris en compte dans l'évaluation.
pour cette partie du site web, il est nécessaire d'autoriser Chrome à lire les fichiers locaux.

Pour réaliser ce tableau, j'ai utilisé la fonction Papa Parse qui permet de lire les fichiers CSV. En effet j'ai un tableau excel de toute mes sources pour mon mémoire, je l'ai transformé en fichier CSV et convertit en language UTF-8 pour l'utiliser correctement.
...
Au vue de la taille du code, je resterais relativement éliptique sur certaines parties pour me concentrer sur le principal.
...
Tout en bas du tableau, un message indique le nombre de source trouvé lors de la recherche grace à cette fonction : 
...
Si la colonne "explication" est vide, c'est parce que je n'ai pas trouvé nécessaire d'en inserer. Ce n'est pas une erreur du code.
...
Enfin après la création des conteneurs et des menus déroulant (dont je n'ai montré qu'un extrait), le coeur de la fonction de filtrage peut s'appliquer :
````java
...
Après les fonction établissants le filtrage, il reste de la ligne 242 à la ligne 310 les fonction de vérification et d'affichage des données, et enfin la fonction de mise à jours du compteur.
